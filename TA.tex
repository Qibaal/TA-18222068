...
\usepackage{babel} % untuk bahasa Indonesia
\babelprovide[import, main]{indonesian} % memuat data bahasa Indonesia (workaround ketidakcocokan versi babel/tracklang di beberapa distribusi TeX Live)
...
\usepackage{float} % for the [H] "here, definitely" float placement specifier
...
\usepackage{xltabular} % for long tables with adjustable-width columns
\usepackage{multirow} % for multi-row table cells
\usepackage{xcolor} % for color definitions
\usepackage[title,titletoc]{appendix} % for appendix management
\usepackage{csquotes} % for context-sensitive quotation facilities

% todonotes tidak tersedia di instalasi TeX Live ini (butuh hak admin untuk tlmgr),
% sehingga placeholder ditandai dengan makro sederhana berbasis xcolor.
\newcommand{\placeholder}[1]{\par\noindent\fcolorbox{red}{yellow!15}{\parbox{0.97\textwidth}{\textbf{\textcolor{red}{[PLACEHOLDER]}} #1}}\par}

\setmainfont{Times New Roman} % set main font to Times New Roman
\onehalfspacing % spasi 1.5
\raggedbottom % cegah LaTeX meregangkan spasi vertikal untuk memenuhi halaman (menghindari whitespace besar di sekitar gambar/tabel besar)
