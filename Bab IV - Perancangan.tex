...
Komponen \textit{Database} merupakan lapisan penyimpanan data terpusat pada WMC. Komponen ini menyimpan seluruh data operasional gudang, mencakup detail barang, \textit{layout} penyimpanan, data pengguna, sesi autentikasi, dan status pengiriman robot. Seluruh layanan WMC lainnya berinteraksi dengan komponen ini melalui mekanisme \textit{query} yang terstruktur. Gambar \ref{fig:database-arch} mengilustrasikan arsitektur komponen \textit{Database}, dan Tabel \ref{tab:database-io} merangkum fungsi, masukan, dan keluaran komponen \textit{Database}.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Database WMC}
  \caption{Arsitektur Komponen \textit{Database} WMC (Level 2)}
...
Komponen \textit{Goods Detail Service} bertanggung jawab mengelola data master seluruh barang yang tersimpan di gudang. Komponen ini menjadi antarmuka antara \textit{Database} dan komponen lain yang memerlukan informasi barang, mencakup penyimpanan, pembaruan, dan pengambilan informasi secara terperinci. Gambar \ref{fig:gds-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:gds-io} merangkum fungsi, masukan, dan keluaran komponen ini.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Goods Detail Service WMC}
  \caption{Arsitektur Komponen \textit{Goods Detail Service} WMC (Level 2)}
  \label{fig:gds-arch}
\end{figure}
...
Komponen \textit{Fleet Order Service} merupakan \textit{gateway} utama antara sistem WMC dan komponen \textit{Fleet Controller} pada robot. Komponen ini mengolah seluruh logika pembuatan dan pembatalan \textit{order}, pengaturan tugas robot, serta pemantauan dan sinkronisasi status tugas secara \textit{real-time}. Gambar \ref{fig:fos-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:fos-io} merangkum fungsi, masukan, dan keluaran komponen ini.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Fleet Order Service WMC}
  \caption{Arsitektur Komponen \textit{Fleet Order Service} WMC (Level 2)}
  \label{fig:fos-arch}
\end{figure}
...
Komponen \textit{Authentication Service} bertanggung jawab memvalidasi identitas pengguna yang mengakses sistem WMC. Komponen ini menerima kredensial dari antarmuka pengguna, memverifikasinya terhadap data pada \textit{Database}, kemudian menerbitkan \textit{Bearer Token} sebagai bukti otorisasi untuk seluruh permintaan berikutnya. Gambar \ref{fig:auth-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:auth-io} merangkum fungsi, masukan, dan keluaran komponen ini.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Authentication Service WMC}
  \caption{Arsitektur Komponen \textit{Authentication Service} WMC (Level 2)}
  \label{fig:auth-arch}
\end{figure}
...
\subsection{Arsitektur Level 1 --- Fleet Controller}
...
Pada Level 1, \textit{Fleet Controller} dipandang sebagai satu modul tunggal yang menerima instruksi \textit{order} dan data robot dari WMC, kemudian menghasilkan perintah eksekusi bagi robot serta status terkini yang dikembalikan ke WMC. Gambar \ref{fig:fc-level1-arch} mengilustrasikan arsitektur \textit{Fleet Controller} pada Level 1, dan Tabel \ref{tab:fc-level1} merangkum fungsi, masukan, dan keluaran \textit{Fleet Controller} pada Level 1.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Fleet Controller Level 1}
  \caption{Arsitektur \textit{Fleet Controller} (Level 1)}
  \label{fig:fc-level1-arch}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Fungsi, masukan, dan keluaran \textit{Fleet Controller} (Level 1)}
  \label{tab:fc-level1}
  \begin{tabularx}{\textwidth}{ | l | X | }
    \hline
    \textbf{Elemen} & \textbf{Deskripsi} \\
...
\subsection{Arsitektur Level 2 --- Komponen Fleet Controller}
Pada Level 2, \textit{Fleet Controller} didekomposisi menjadi lima komponen perangkat lunak, yaitu \textit{Communication Service}, \textit{Fleet Manager}, \textit{Local Database}, \textit{Robot Path Planner}, dan \textit{Localisation System}. Keempat komponen pertama merupakan hasil rancangan kelompok TA252601001 sebagaimana didokumentasikan pada dokumen B300, sedangkan komponen \textit{Localisation System} merupakan kontribusi individu penulis yang menggantikan pendekatan lokalisasi berbasis QR Code + \textit{Line Following} pada rancangan awal dengan pendekatan AprilTag + \textit{Overhead Fisheye Camera} (lihat Subbab \ref{sec:batasan-masalah}).

\subsubsection{Communication Service}
\textit{Communication Service} pada \textit{Fleet Controller} berfungsi sebagai penghubung komunikasi dua arah antara sistem pengendali dan robot, mengirimkan perintah kendali serta menerima data status robot secara \textit{real-time} melalui media komunikasi nirkabel. Gambar \ref{fig:comm-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:comm-io} merangkum fungsi, masukan, dan keluarannya.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Communication Service Fleet Controller}
  \caption{Arsitektur Komponen \textit{Communication Service} \textit{Fleet Controller} (Level 2)}
  \label{fig:comm-arch}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Fungsi, masukan, dan keluaran \textit{Communication Service}}
  \label{tab:comm-io}
  \begin{tabularx}{\textwidth}{ | l | X | }
    \hline
    \textbf{Elemen} & \textbf{Deskripsi} \\
    \hline
    Fungsi & Menyediakan komunikasi dua arah antara \textit{Fleet Controller} dan robot untuk mengirimkan perintah kendali serta menerima data status robot secara \textit{real-time}. \\
    \hline
    Masukan & 1. Perintah Robot --- instruksi kendali yang akan diteruskan ke robot, seperti perintah bergerak atau menjalankan tugas tertentu, diterima dari \textit{Fleet Manager}. 2. Data Robot --- informasi status robot (posisi, kondisi, dan hasil eksekusi perintah) yang diterima dari robot melalui media komunikasi. \\
    \hline
    Luaran & 1. Perintah Robot --- diteruskan ke unit robot melalui media komunikasi yang digunakan. 2. Data Robot --- diteruskan ke \textit{Fleet Manager} dan \textit{Local Database} untuk diproses lebih lanjut. \\
    \hline
  \end{tabularx}
\end{table}

\subsubsection{Fleet Manager}
\textit{Fleet Manager} berfungsi mengoordinasikan operasional robot berdasarkan \textit{order} yang diterima, menentukan penugasan robot, serta memperbarui status \textit{order} dan data barang secara terintegrasi. Gambar \ref{fig:fleetmanager-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:fleetmanager-io} merangkum fungsi, masukan, dan keluarannya.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Fleet Manager Fleet Controller}
  \caption{Arsitektur Komponen \textit{Fleet Manager} \textit{Fleet Controller} (Level 2)}
  \label{fig:fleetmanager-arch}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Fungsi, masukan, dan keluaran \textit{Fleet Manager}}
  \label{tab:fleetmanager-io}
  \begin{tabularx}{\textwidth}{ | l | X | }
    \hline
    \textbf{Elemen} & \textbf{Deskripsi} \\
    \hline
    Fungsi & Mengoordinasikan operasional robot berdasarkan \textit{order} yang diterima, menentukan penugasan robot, serta memperbarui status \textit{order} dan data barang secara terintegrasi. \\
    \hline
    Masukan & 1. Data Robot --- informasi kondisi dan ketersediaan robot, diterima dari \textit{Communication Service}. 2. \textit{Order Command} --- instruksi pemrosesan \textit{order}, diterima dari \textit{Fleet Order Service} WMC. 3. \textit{Generate Route} --- hasil perencanaan rute robot, diterima dari \textit{Robot Path Planner}. \\
    \hline
    Luaran & 1. Perintah Robot --- instruksi eksekusi tugas dikirim ke \textit{Communication Service}. 2. Data Robot (terbarui) --- dikirim ke \textit{Local Database}. 3. Status \textit{Order} \& Detail \textit{Order} --- status pesanan dikirim kembali ke \textit{Fleet Order Service} WMC, detail \textit{order} diteruskan ke \textit{Robot Path Planner} sebagai dasar perencanaan rute. \\
    \hline
  \end{tabularx}
\end{table}

\subsubsection{Local Database}
\textit{Local Database} berfungsi sebagai penyimpanan lokal pada \textit{Fleet Controller} untuk data barang dan data robot yang diperbarui secara berkala, sehingga pengambilan keputusan operasional dapat dilakukan secara cepat tanpa bergantung sepenuhnya pada \textit{Database} WMC. Gambar \ref{fig:localdb-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:localdb-io} merangkum fungsi, masukan, dan keluarannya.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Local Database Fleet Controller}
  \caption{Arsitektur Komponen \textit{Local Database} \textit{Fleet Controller} (Level 2)}
  \label{fig:localdb-arch}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Fungsi, masukan, dan keluaran \textit{Local Database}}
  \label{tab:localdb-io}
  \begin{tabularx}{\textwidth}{ | l | X | }
    \hline
    \textbf{Elemen} & \textbf{Deskripsi} \\
    \hline
    Fungsi & Menyimpan dan menyediakan data barang serta data robot yang diperbarui oleh sistem \textit{Fleet Controller}. \\
    \hline
    Masukan & 1. Pembaruan Data Barang --- informasi perubahan status, jumlah, atau lokasi barang di gudang. 2. Pembaruan Data Robot --- informasi terbaru terkait kondisi dan aktivitas robot. \\
    \hline
    Luaran & 1. Data Barang --- informasi barang yang tersimpan dan siap digunakan oleh modul lain dalam sistem. 2. Data Robot --- data robot terkini yang digunakan sebagai referensi \textit{Fleet Manager} dan \textit{Robot Path Planner}. \\
    \hline
  \end{tabularx}
\end{table}

\subsubsection{Robot Path Planner}
\textit{Robot Path Planner} berfungsi merencanakan rute pergerakan robot berdasarkan data robot, data barang, dan detail \textit{order} agar proses \textit{picking} dan \textit{storing} berjalan optimal. Gambar \ref{fig:pathplanner-arch} mengilustrasikan arsitektur komponen ini, dan Tabel \ref{tab:pathplanner-io} merangkum fungsi, masukan, dan keluarannya.

\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Robot Path Planner Fleet Controller}
  \caption{Arsitektur Komponen \textit{Robot Path Planner} \textit{Fleet Controller} (Level 2)}
  \label{fig:pathplanner-arch}
\end{figure}

\begin{table}[h]
  \centering
  \caption{Fungsi, masukan, dan keluaran \textit{Robot Path Planner}}
  \label{tab:pathplanner-io}
  \begin{tabularx}{\textwidth}{ | l | X | }
    \hline
    \textbf{Elemen} & \textbf{Deskripsi} \\
    \hline
    Fungsi & Merencanakan rute pergerakan robot berdasarkan data robot, data barang, dan detail \textit{order} agar proses pengambilan dan pengantaran barang berjalan optimal. \\
    \hline
    Masukan & 1. Data Barang --- informasi lokasi dan karakteristik barang yang akan diambil atau dikirim, diterima dari \textit{Local Database}. 2. Data Robot --- informasi posisi dan kondisi robot yang akan digunakan. 3. Detail \textit{Order} --- informasi tujuan dan kebutuhan \textit{order} yang harus dipenuhi, diterima dari \textit{Fleet Manager}. \\
    \hline
    Luaran & 1. \textit{Generate Route} --- rute pergerakan robot yang telah direncanakan dan siap digunakan oleh \textit{Fleet Manager}. \\
    \hline
  \end{tabularx}
\end{table}

\subsubsection{Localisation System}
...
\begin{figure}[h]
  \centering
  \arsfig{Arsitektur Komponen Localisation System Fleet Controller}
  \caption{Arsitektur Komponen \textit{Localisation System} \textit{Fleet Controller} (Level 2)}
